...
Rectangle {
                   ...
                   ColumnLayout {
                       ...
                       RowLayout {
                           ...
                           Item {
                               ...
                               // 显示10个字段
                               property var horHeader: ["","序号", "操作方式"]
                       ...
                       RowLayout {
                           ...
                           Button {
                               ...
                               text: "返回"
                               onClicked: {
                                   addUavModelOperationWayPopup.close()// 或myPopup.visible = false
           ...
           function saveUavComponentData(){
               ...
               if(uavComponeData.uavComponeName.length  === 0){
                   ...
                   let insertResult = uavModelOperationWayDaoModel.insertModelOperationWayDate(uavComponeData)
           ...
           function recieveUavComponentAllData (){
                       var receiveData = uavModelOperationWayDaoModel.selectModelOperationWayAllData()
           ...
           function deleteUavComponentData(){
               ...
               // 打印当前函数的名称
                console.log("当前函数名称:", arguments.callee.name);
               if(selectData === tableModel.rowCount){
                   warningItem.text = "操控方式数据不能全部删除!"
                   warningPopup.open()
                   // 2秒后自动关闭
                   autoCloseTimer.start()
               }else{
                   let result = uavModelOperationWayDaoModel.deleteModelOperationWayDate(selectedRowsData)
                   recieveUavComponentAllData()
                   if(result === true){
           ...
           function updateUavComponentData(){
               ...
               let result = uavModelOperationWayDaoModel.updateModelOperationWayDate(resultData)
               recieveUavComponentAllData()
               if(result === true){

...
#include "AmmunitionAttackTargetTypeEntity-odb.hxx"
#include <stdexcept>
// ODB 头文件
...
#include <QJsonObject>
#include <QJsonValue>
AmmoAttackTargetDao::AmmoAttackTargetDao(QObject* parent) : QObject(parent) {
...
QJsonArray AmmoAttackTargetDao::selectAmmoAttackTargetAllData()
{
    ...
    try{
        ...
        using query_t = odb::query<AmmoAttackTargetTypeEntity>;

        odb::result<AmmoAttackTargetTypeEntity> result = db.query<AmmoAttackTargetTypeEntity>(query_t::ammoAttackTargetStatus == true);
        ...
        for (AmmoAttackTargetTypeEntity entity : result) { //auto&& entity : result) {
            QJsonObject obj;
            qDebug() << "Processing record ID:" << entity.id_;  // 输出当前记录ID
            // 手动转换实体到 JSON（需要根据实际字段补充）
            obj["index"] = sum;
            obj["recordId"] = QString::number(entity.id_);
            obj["ammoComponeCode"] = QString::fromStdString(entity.ammoAttackTargetCode_);
            obj["ammoComponeName"] = QString::fromStdString(entity.ammoAttackTargetName_);
...
QJsonArray AmmoAttackTargetDao::queryAmmoAttackTargetData(const QJsonObject &uavModel)
{
    ...
    try {
        ...
        // 3. 从JSON创建实体对象
        AmmoAttackTargetTypeEntity entity;
        ...
        if (uavModel["ammoComponeName"] == ""){
            ...
            if (uavModel.contains("ammoComponeName") && uavModel["ammoComponeName"].isString()) {
                ...
                q = q && (query_t::ammoAttackTargetName == uav_type.toStdString());
            }
        }
        odb::result<AmmoAttackTargetTypeEntity> result = db.query<AmmoAttackTargetTypeEntity>(q);
        qDebug() << "Query returned" << result.size() << "records";  // 添加此行
        // 关键修正2：遍历所有结果
        int sum = 0;
        bool checked = false;
        ...
        for (AmmoAttackTargetTypeEntity entity : result) { //auto&& entity : result) {
            ...
            obj["ammoComponeCode"] = QString::fromStdString(entity.ammoAttackTargetCode_);
            obj["ammoComponeName"] = QString::fromStdString(entity.ammoAttackTargetName_);
            obj["ammoStatus"] = QString::number(entity.ammoAttackTargetStatus_);
            obj["checked"] = checked;
            sum++;
            qDebug()<<"uavModelAllDatauavcreat:";
...
bool AmmoAttackTargetDao::updateAmmoAttackTargetData(const QJSValue &selectedData)
{
    try {
        // 1. 建立数据库连接
        ...
        // 3. 从JSON创建实体对象
        AmmoAttackTargetTypeEntity entity;
        typedef odb::query<AmmoAttackTargetTypeEntity> query;
...
bool AmmoAttackTargetDao::deleteAmmoAttackTargetData(const QJSValue &selectedData)
{
    try {
        // 1. 建立数据库连接
        qDebug() << "Connecting to database...";
...
bool AmmoAttackTargetDao::insertAmmoAttackTargetData(const QJsonObject &object)
{
    ...
    try {
        ...
        // 4. 映射JSON字段到实体属性
        // 基础字段
        entity.ammoAttackTargetCode_ = object["ammoComponeCode"].toString().toStdString();//.toInt();
        entity.ammoAttackTargetName_ = object["ammoComponeName"].toString().toStdString();
        entity.ammoAttackTargetStatus_ = object["ammoStatus"].toBool();

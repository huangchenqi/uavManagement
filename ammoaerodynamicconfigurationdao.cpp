...
#include "AmmoAerodynamicConfiguration.h"
#include "AmmoAerodynamicConfiguration-odb.hxx"
#include <stdexcept>
// ODB 头文件
...
#include <QDebug>
#include <QDateTime>
AmmoAerodynamicConfigurationDao::AmmoAerodynamicConfigurationDao(QObject* parent) : QObject(parent) {
...
QJsonArray AmmoAerodynamicConfigurationDao::selectAmmoAerodynamicConfigurationAllData()
{
    ...
    try{
        ...
        using query_t = odb::query<AmmoAerodynamicConfiguration>;

        odb::result<AmmoAerodynamicConfiguration> result = db.query<AmmoAerodynamicConfiguration>(query_t::status == true);
        ...
        for (AmmoAerodynamicConfiguration entity : result) { //auto&& entity : result) {
            QJsonObject obj;
            qDebug() << "Processing record ID:" << entity.id_;  // 输出当前记录ID
            // 手动转换实体到 JSON（需要根据实际字段补充）
            obj["index"] = sum;
            obj["recordId"] = QString::number(entity.id_);
            obj["ammoComponeCode"] = QString::fromStdString(entity.aerodynamicConfigurationCode_);
            obj["ammoComponeName"] = QString::fromStdString(entity.aerodynamicConfigurationName_);
...
QJsonArray AmmoAerodynamicConfigurationDao::queryAmmoAerodynamicConfigurationData(const QJsonObject &uavModel)
{
    ...
    try {
        ...
        // 3. 从JSON创建实体对象
        AmmoAerodynamicConfiguration entity;
        ...
        if (uavModel["ammoComponeName"] == ""){
            ...
            if (uavModel.contains("ammoComponeName") && uavModel["ammoComponeName"].isString()) {
                ...
                q = q && (query_t::aerodynamicConfigurationName == uav_type.toStdString());
            }
        }
        odb::result<AmmoAerodynamicConfiguration> result = db.query<AmmoAerodynamicConfiguration>(q);
        qDebug() << "Query returned" << result.size() << "records";  // 添加此行
        // 关键修正2：遍历所有结果
        int sum = 0;
        bool checked = false;
        ...
        for (AmmoAerodynamicConfiguration entity : result) { //auto&& entity : result) {
            ...
            obj["ammoComponeCode"] = QString::fromStdString(entity.aerodynamicConfigurationCode_);
            obj["ammoComponeName"] = QString::fromStdString(entity.aerodynamicConfigurationName_);
            obj["ammoStatus"] = QString::number(entity.status_);
            obj["checked"] = checked;
            sum++;
            qDebug()<<"uavModelAllDatauavcreat:";
...
bool AmmoAerodynamicConfigurationDao::updateAmmoAerodynamicConfigurationData(const QJSValue &selectedData)
{
    ...
    try {
        ...
        // 3. 从JSON创建实体对象
        AmmoAerodynamicConfiguration entity;
        typedef odb::query<AmmoAerodynamicConfiguration> query;
        ...
        for (const QVariant &item : dataList) {
            ...
            entity.aerodynamicConfigurationName_= ammoNameStr.toStdString();
            //entity.ammoAttackTargetStatus_ = ammoStatusStr;
...
bool AmmoAerodynamicConfigurationDao::deleteAmmoAerodynamicConfigurationData(const QJSValue &selectedData)
{
    ...
    try {
        ...
        // 2. 创建事务
        odb::transaction trans(db.begin());
        qDebug() << "Transaction delete deleteAmmoAttackTargetData started";
        // 3. 从JSON创建实体对象
        AmmoAerodynamicConfiguration entity;
        //typedef odb::query<AmmoAerodynamicConfiguration> query;
        // 将 QJSValue 转换为 QVariantList
...
bool AmmoAerodynamicConfigurationDao::insertAmmoAerodynamicConfigurationData(const QJsonObject &object)
{
    ...
    try {
        ...
        // 4. 映射JSON字段到实体属性
        // 基础字段
        entity.aerodynamicConfigurationCode_ = object["ammoComponeCode"].toString().toStdString();//.toInt();
        entity.aerodynamicConfigurationName_ = object["ammoComponeName"].toString().toStdString();
        entity.status_ = object["ammoStatus"].toBool();

...
#include "AmmunitionGuidanceTypeEntity-odb.hxx"
#include <stdexcept>
// ODB 头文件
...
#include <QJsonObject>
#include <QJsonValue>
AmmoGuidanceTypeDao::AmmoGuidanceTypeDao(QObject* parent) : QObject(parent) {
...
QJsonArray AmmoGuidanceTypeDao::selectAmmoGuidanceTypeAllData()
{
    ...
    try{
        ...
        using query_t = odb::query<AmmoGuidanceEntity>;

        odb::result<AmmoGuidanceEntity> result = db.query<AmmoGuidanceEntity>(query_t::ammoGuidanceStatus == true);
        ...
        for (AmmoGuidanceEntity entity : result) { //auto&& entity : result) {
            QJsonObject obj;
            qDebug() << "Processing record ID:" << entity.id_;  // 输出当前记录ID
            // 手动转换实体到 JSON（需要根据实际字段补充）
            obj["index"] = sum;
            obj["recordId"] = QString::number(entity.id_);
            obj["ammoComponeCode"] = QString::fromStdString(entity.ammoGuidanceCode_);
            obj["ammoComponeName"] = QString::fromStdString(entity.ammoGuidanceName_);
...
QJsonArray AmmoGuidanceTypeDao::selectSomeAmmoGuidanceTypeData(const QJsonObject& selectedData)
{
    QJsonArray uavToMountArray;//yQJsonObject
    ...
    try {
        ...
        if (selectedData["ammoComponeName"] == ""){
            ...
            if (selectedData.contains("ammoComponeName") && selectedData["ammoComponeName"].isString()) {
                ...
                q = q && (query_t::ammoGuidanceName == uav_type.toStdString());
            }
        }
        odb::result<AmmoGuidanceEntity> result = db.query<AmmoGuidanceEntity>(q);
        qDebug() << "Query returned" << result.size() << "records";  // 添加此行
        // 关键修正2：遍历所有结果
        int sum = 0;
        bool checked = false;
        ...
        for (AmmoGuidanceEntity entity : result) { //auto&& entity : result) {
            ...
            obj["ammoComponeCode"] = QString::fromStdString(entity.ammoGuidanceCode_);
            obj["ammoComponeName"] = QString::fromStdString(entity.ammoGuidanceName_);
            obj["ammoStatus"] = QString::number(entity.ammoGuidanceStatus_);
            obj["checked"] = checked;
            sum++;
            qDebug()<<"uavModelAllDatauavcreat:";
...
bool AmmoGuidanceTypeDao::updateAmmoGuidanceTypeData(const QJSValue &selectedData)
{
    try {
        // 1. 建立数据库连接
        ...
        // 3. 从JSON创建实体对象
        AmmoGuidanceEntity entity;
        typedef odb::query<AmmoGuidanceEntity> query;
        ...
        for (const QVariant &item : dataList) {
            ...
            //entity.ammoGuidanceStatus_ = ammoStatusStr;
...
bool AmmoGuidanceTypeDao::deleteAmmoGuidanceTypeData(const QJSValue &selectedData)
{
    try {
        // 1. 建立数据库连接
        qDebug() << "Connecting to database...";
...
bool AmmoGuidanceTypeDao::insertAmmoGuidanceTypeData(const QJsonObject &object)
{
    ...
    try {
        ...
        // 4. 映射JSON字段到实体属性
        // 基础字段
        entity.ammoGuidanceCode_ = object["ammoComponeCode"].toString().toStdString();//.toInt();
        entity.ammoGuidanceName_ = object["ammoComponeName"].toString().toStdString();
        entity.ammoGuidanceStatus_ = object["ammoStatus"].toBool();

...
#include "AmmunitionTypeEntity-odb.hxx"
#include <stdexcept>
// ODB 头文件
...
QJsonArray AmmoTypeDao::selectAmmoTypeAllData()
{
    ...
    try{
        ...
        using query_t = odb::query<AmmoTypeEntity>;
        ...
        for (AmmoTypeEntity entity : result) { //auto&& entity : result) {
            QJsonObject obj;
            qDebug() << "Processing record ID:" << entity.id_;  // 输出当前记录ID
            // 手动转换实体到 JSON（需要根据实际字段补充）
            obj["index"] = sum;
            obj["recordId"] = QString::number(entity.id_);
            obj["ammoComponeCode"] = QString::fromStdString(entity.ammotypeCode_);
            obj["ammoComponeName"] = QString::fromStdString(entity.ammoTypeName_);
    ...
    try {
        ...
        if (uavModel["ammoComponeName"] == ""){
            ...
            if (uavModel.contains("ammoComponeName") && uavModel["ammoComponeName"].isString()) {
                ...
                q = q && (query_t::ammoTypeName == uav_type.toStdString());
            }
        }
        odb::result<AmmoTypeEntity> result = db.query<AmmoTypeEntity>(q);
        qDebug() << "Query returned" << result.size() << "records";  // 添加此行
        // 关键修正2：遍历所有结果
        int sum = 0;
        bool checked = false;
        ...
        for (AmmoTypeEntity entity : result) { //auto&& entity : result) {
            ...
            obj["ammoComponeCode"] = QString::fromStdString(entity.ammotypeCode_);
            obj["ammoComponeName"] = QString::fromStdString(entity.ammoTypeName_);
            obj["ammoStatus"] = QString::number(entity.ammoTypeStatus_);
            obj["checked"] = checked;
            sum++;
            qDebug()<<"uavModelAllDatauavcreat:";
...
bool AmmoTypeDao::updateAmmoTypeData(const QJSValue &selectedData)
{
    try {
        ...
        // 3. 从JSON创建实体对象
        AmmoTypeEntity entity;
        typedef odb::query<AmmoTypeEntity> query;
        ...
        for (const QVariant &item : dataList) {
            ...
            //entity.ammotypeCode_ = ammoIdStr.toInt();
            entity.ammoTypeName_= ammoNameStr.toStdString();
            //entity.ammoTypeStatus_ = ammoStatusStr;
...
bool AmmoTypeDao::deleteAmmoTypeData(const QJSValue &selectedData)
{
    try {
        ...
        for (const QVariant &item : dataList) {
            ...
            // 4. 修改数据
            db.update(entity);
            // auto rst = db.erase_query<AmmoTypeEntity>(//db.erase_query<UavModelEntity>
            //     query::id == recordId
            //     && query::ammoTypeName == ammoNameStr.toStdString().c_str()
...
bool AmmoTypeDao::insertAmmoTypeData(const QJsonObject &object)
{
    ...
    try {
        ...
        // 4. 映射JSON字段到实体属性
        // 基础字段
        entity.ammotypeCode_ = object["ammoComponeCode"].toString().toStdString();//.toInt();
        entity.ammoTypeName_ = object["ammoComponeName"].toString().toStdString();
        entity.ammoTypeStatus_ = object["ammoStatus"].toBool();

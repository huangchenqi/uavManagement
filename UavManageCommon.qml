...
Rectangle {
       ...
       //                       }
                              ComboBox {
                                  ...
                                  height: 40
                                  font.pointSize: 12
                                  model: uavManagementroot.uavTypeLoad//["全部","侦察型无人机","攻击型无人机","查打一体无人机"]
...
//                          }
                      ...
                      RowLayout {
                          ...
                          Item {
                              ...
                              // 显示10个字段
                              property var horHeader: ["","序号", "无人机机型", "无人机名称", "无人机型号", "挂载内容",
          ...
          Component {
           ...
           function findRecordUavIdByName(uavData,name) {
                   for (var i = 0; i < uavData.length; i++) {
                       if (uavData[i].uavComponeName === name) {
                           return uavData[i].recordId;
                       }
                   }
           ...
           function queryConditionUavModelData(){
               ...
               console.log("uavManagementroot.uavTypeOriginStr"+JSON.stringify(uavManagementroot.uavTypeOriginStr))
               var uavIdStr  = uavIdSelect.currentText
               console.log("uavManagementrootr"+uavIdStr)
               uavData.uavType = modelSelector.currentText
               uavData.uavId = findRecordUavIdByName(uavManagementroot.uavTypeOriginStr,uavIdStr)
               console.log("uavData.uavId"+uavData.uavId)
           ...
           function  transformUavType(data){
               var names = [];
               for (var i = 0; i < data.length; i++) {
                   names.push(data[i].uavComponeName);


               }
               console.log("uavModcccccelTypeDaoTableModel"+names)
               return names;
           }
           function loaduavIdSelect(){
              var loaduavId = uavModelTypeDaoTableModel.selectUavModelTypeData()
               ...
               var selectedRowsData = [];
                   for (var i = 0; i < tableModel.rowCount; i++) {
                       ...
                       if (tableModel.rows[i].checked) {
                           var rowData = {
                               recordId: parseInt(tableModel.rows[i].recordId, 10),//recordId:tableModel.rows[i].recordId,
                   ...
                   console.log("Selected Rows JSON:", selectedRowsJson);
                    if (selectedRowsData.length === 0) {
                        warningPopup.open()
                        warningItem.text = "删除数据不能为空"
                        // 2秒后自动关闭
                        autoCloseTimer.start()
                        return false;
                    }else{
                        //console.log("DeleteUavData"+selectedRowsJson)
                        let result = uavModelDaoTable.deleteModelDate(selectedRowsData)
                        loadUavAllData()
                        if(result === true){

...
#include <QJsonObject>
#include <QJsonValue>
UavModelOperationWayDao::UavModelOperationWayDao(QObject* parent) : QObject(parent) {
...
bool UavModelOperationWayDao::updateModelOperationWayDate(const QJSValue &selectedData)
{
    try {
        // 1. 建立数据库连接
        ...
        // 3. 从JSON创建实体对象
        UavModelOpreationWayEntity entity;
        typedef odb::query<UavModelOpreationWayEntity> query;
        ...
        for (const QVariant &item : dataList) {
            ...
            entity.operationWayName_ = uavComponeNameStr.toStdString();
...
bool UavModelOperationWayDao::deleteModelOperationWayDate(const QJSValue &selectedData)
{
    try {
        // 1. 建立数据库连接
        ...
        for (const QVariant &item : dataList) {
            ...
            QString uavComponeNameStr = dataMap["uavComponeName"].toString();
            //QString uavComponeCodeStr = dataMap["uavComponeCode"].toString();
            auto rst = db.erase_query<UavModelOpreationWayEntity>(//db.erase_query<UavModelEntity>
                query::id == recordId
                && query::operationWayName == uavComponeNameStr.toStdString().c_str()
...
bool UavModelOperationWayDao::insertModelOperationWayDate(const QJsonObject &object)
{
    ...
    try {
        ...
        // 4. 映射JSON字段到实体属性
        // 基础字段
        entity.operationWayCode_ = object["uavComponeCode"].toString().toStdString();
        entity.operationWayName_ = object["uavComponeName"].toString().toStdString();
...
QJsonArray UavModelOperationWayDao::selectModelOperationWayAllData()
{
    ...
    try{
        ...
        // 2. 创建事务
        odb::transaction trans(db.begin());
        qDebug() << "Transaction select all UavModelOpreationWayEntity started";
        // 关键修正1：使用 query<UavModelOpreationWayEntity> 获取结果集
        using query_t = odb::query<UavModelOpreationWayEntity>;

        odb::result<UavModelOpreationWayEntity> result = db.query<UavModelOpreationWayEntity>(query_t::true_expr);
        ...
        for (UavModelOpreationWayEntity entity : result) { //auto&& entity : result) {
            QJsonObject obj;
            qDebug() << "Processing record ID:" << entity.id_;  // 输出当前记录ID
            // 手动转换实体到 JSON（需要根据实际字段补充）
            obj["index"] = sum;
            obj["recordId"] = QString::number(entity.id_);
            obj["uavComponeCode"] = QString::fromStdString(entity.operationWayCode_);
            obj["uavComponeName"] = QString::fromStdString(entity.operationWayName_);
            obj["checked"] = checked;
            sum++;
